    ...
    *,
    ...
    async def _chat_post_with_block_fallback(client, kwargs: Dict[str, Any]):
        ...
        try:
            ...
            raise
        response_payload = _slack_response_payload(response)
        if (
            not response_payload.get("ok", True)
            and kwargs.get("blocks")
            and SlackAdapter._is_block_payload_rejection(response_payload)
        ):
            fallback_kwargs = dict(kwargs)
            fallback_kwargs.pop("blocks", None)
            return await client.chat_postMessage(**fallback_kwargs)
        ...
        if text_to_send.strip():
            ...
            try:
                post_payload = _slack_response_payload(
                    await _chat_post_with_block_fallback(client, post_kwargs)
                )
                ...
                if caption_pending:
                    ...
                    try:
                        ...
                        fb = _slack_response_payload(
                            await _chat_post_with_block_fallback(
