...
def _notification_event_type(evt: dict) -> str:
    """Normalize legacy/malformed notification types like the formatter does."""
    raw_type = evt.get("type", "completion")
    return raw_type if isinstance(raw_type, str) else "completion"
    ...
    def _owner_sid_for_process(session) -> str:
        ...
        with _sessions_lock:
            if origin_ui_session_id:
                origin = _sessions.get(origin_ui_session_id)
                if origin is not None and not origin.get("_finalized"):
            ...
            matches = [
                ...
                if (
                    not tui_session.get("_finalized")
                    and str(tui_session.get("session_key") or "") == session_key
                )
            ]
            if len(matches) == 1:
                return matches[0]
